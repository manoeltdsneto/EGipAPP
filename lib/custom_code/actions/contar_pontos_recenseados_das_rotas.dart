...
Future<int> contarPontosRecenseadosDasRotas() async {
  ...
  // 2. Extrair todos os IDs de pontos
  ...
  // 3. Buscar os id_ponto dos recenseados = 'SIM', agrupando
  final idList = pontoIds.join(',');
  final result = await db.rawQuery('''
    SELECT id_ponto FROM pontos
    WHERE is_deleted = 0
      AND UPPER(TRIM(recenseado)) = 'SIM'
      AND id IN ($idList)
    GROUP BY id_ponto
  ''');
