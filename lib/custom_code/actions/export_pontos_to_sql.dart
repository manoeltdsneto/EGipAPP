...
// name: exportPontosNaoSincronizadosToSql
// description: Exporta registros da tabela 'pontos' onde sincronizado = 0 para um arquivo .sql (INSERTs) e salva via picker do sistema.
// inputs: none
...
Future<String?> exportPontosToSql() async {
  // 1) Abre o DB
  ...
  // Garante que a coluna 'sincronizado' existe
  if (!columns.contains('sincronizado')) {
    throw Exception(
        "Coluna 'sincronizado' não existe na tabela 'pontos'. Colunas: ${columns.join(', ')}");
  }

  // 3) Buscar somente registros onde sincronizado = 0
  //    (se sua coluna estiver como bool, o SQLite normalmente armazena 0/1 também)
  ...
  if (rows.isEmpty) {
    buf.writeln('-- Nenhum registro encontrado com sincronizado = 0.');
  } else {
  ...
  final fileName = 'pontos_nsync_${_formatDateTime(DateTime.now())}.sql';
